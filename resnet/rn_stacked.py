...
for i in range(10):
  ...
  print("Total images in FA_nodule ",len(FA_nodule))
  NIFTP_nodule=os.listdir(path+"/content/NIFTP nodule")
  print("Total images in NIFTP_nodule ",len(NIFTP_nodule))
  PTC_nodule=os.listdir(path+"/content/PTC_nodule")
  print("Total images in PTC_nodule ",len(PTC_nodule))
  ...
  for dir in os.listdir(path+"/content/FA_capsule"):
      patient_id=dir.split("_")[0]
  ...
  for dir in os.listdir(path+"/content/NIFTP capsule"):
      patient_id=dir.split("_")[0]
  ...
  for dir in os.listdir(path+"/content/PTC_capsule"):
      patient_id=dir.split("_")[0]
  ...
  for i in FA_capsule:
    ...
    for j in FA_nodule:
      if k in j:
        image1=tf.keras.preprocessing.image.load_img(path+"/content/FA_nodule/"+j, color_mode='rgb', target_size= (112,112))
        image2=tf.keras.preprocessing.image.load_img(path+"/content/FA_capsule/"+i, color_mode='rgb', target_size= (112,112))
  ...
  for i in NIFTP_capsule:
    ...
    for j in NIFTP_nodule:
      if k in j:
        image1=tf.keras.preprocessing.image.load_img(path+"/content/NIFTP nodule/"+j, color_mode='rgb', target_size= (112,112))
        image2=tf.keras.preprocessing.image.load_img(path+"/content/NIFTP capsule/"+i, color_mode='rgb', target_size= (112,112))
        image1=np.array(image1)
        image2=np.array(image2)
        image= tf.concat([image1,image2],axis=-1)
  ...
  for i in PTC_capsule:
    ...
    for j in PTC_nodule:
      if k in j:
        image1=tf.keras.preprocessing.image.load_img(path+"/content/PTC_nodule/"+j, color_mode='rgb', target_size= (112,112))
        image2=tf.keras.preprocessing.image.load_img(path+"/content/PTC_capsule/"+i, color_mode='rgb', target_size= (112,112))
        image1=np.array(image1)
        image2=np.array(image2)
        image= tf.concat([image1,image2],axis=-1)
  ...
  print('fa-train-',len(fa_train),' fa-val-',len(fa_val),' fa-test-', len(fa_test), ' niftp-train-', len(niftp_train), ' niftp-val-', len(niftp_val), ' niftp-test-', len(niftp_test), ' ptc-train-', len(ptc_train),' ptc-val-', len(ptc_val), ' ptc-test-', len(ptc_test))
  ...
  resnet = tf.keras.applications.resnet50.ResNet50(input_shape=input_shape,weights=None ,include_top=False)
  x = tf.keras.layers.Flatten()(resnet.output)
  prediction = Dense(n_classes, activation = 'softmax')(x)
  rn_model = Model(inputs = resnet.input, outputs=prediction)

  history,pred,prob = run_experiment(rn_model)
  ...
  # plt.show()
  plt.savefig('/workspace/storage/results/densenet/'+dt_string+' resnet_stacked_accuracy')
  plt.show()
  ...
  plt.legend(['train', 'test'], loc='upper left')
  # plt.show()
  plt.savefig('/workspace/storage/results/densenet/'+dt_string+' resnet_stacked_loss')
  plt.show()
  ...
  results_df.to_excel('/workspace/storage/results/densenet/resnet_stacked_results'+dt_string+'.xlsx')
  ...
  # results_df=pd.DataFrame(results_dict, index=[1])
  # results_df.to_excel('/workspace/storage/results/densenet/densenet_resnet_results'+dt_string+'.xlsx')